        ...
        try:
            while True:
                ...
                for line in decoded_data.split("\n"):
                    ...
                    parts = line.split(";")
                    if len(parts) == 3:
                        parent_name, child_name, pid = parts
                        is_exploit = parent_name in SUSPICIOUS_PARENTS and child_name in SUSPICIOUS_CHILDREN
                        ...
                        if is_exploit:
                            print(f"[!!! EXPLOIT BLOCKED !!!] {parent_name} -> {child_name} (PID: {pid})")
                            client_socket.sendall(pid.encode("utf-8"))
                        else:
