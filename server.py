...
def load_config():
    global SUSPICIOUS_PARENTS, SUSPICIOUS_CHILDREN, CRITICAL_SYSTEM_BINARIES, DANGEROUS_KEYWORDS, VT_API_KEY
    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as response:
            if response.status == 200:
                ...
                reason = f"VirusTotal High Detection Ratio ({malicious} engines flagged)"
            ...
            for line in rfile:
                ...
                try:
                    ...
                    if "agent" not in parent_lower and parent_lower != "unknown":
                        if sha256 in MALICIOUS_HASHES:
                        ...
                        elif is_signed and suspicious_dlls:
                            is_exploit = True
                            dll_names = [os.path.basename(path) for path in suspicious_dlls]
                            reason = f"DLL Sideloading (Signed binary mapped unsigned modules: {', '.join(dll_names)})"
                            ...
                            is_malicious, vt_reason = check_virustotal(sha256)
                            if is_malicious:
                                is_exploit = True
                                reason = vt_reason
