...
def load_config():
    global SUSPICIOUS_PARENTS, SUSPICIOUS_CHILDREN, CRITICAL_SYSTEM_BINARIES, DANGEROUS_KEYWORDS, SECRET_TOKEN, ALLOWLIST_HASHES, ALLOWLIST_PATHS
    ...
    try:
        with open(config_path, "r") as f:
            ...
            ALLOWLIST_HASHES = config.get("ALLOWLIST_HASHES", [])
                ...
                for line in decoded_data.split("\n"):
                    ...
                    try:
                        ...
                        if "agent" not in parent_lower and parent_lower != "unknown":
                            ...
                            for ap in ALLOWLIST_PATHS:
                                if ap and ap.lower() in child_path.lower():
                                    is_allowlisted = True
                                    break
                            if sha256 in ALLOWLIST_HASHES:
                                is_allowlisted = True

                            if is_allowlisted:
                                is_exploit = False
                                reason = "Allowlisted Bypass"
                            elif parent_lower in SUSPICIOUS_PARENTS and child_lower in SUSPICIOUS_CHILDREN:
                                is_exploit = True
                                reason = "LOLBAS (Suspicious Parent-Child Link)"
                            elif child_lower in CRITICAL_SYSTEM_BINARIES:
                                is_masquerading = True
                                for sp in ["c:\\windows\\system32", "c:\\windows\\syswow64", "c:\\windows\\explorer.exe", "c:\\windows\\winsxs", "c:\\windows\\servicing"]:
                                    if sp in child_path.lower():
                                        is_masquerading = False
                                        break
                                if is_masquerading:
                                    is_exploit = True
                                    reason = "DLL Sideloading / Path Masquerading Detection"
                                elif not is_signed and sha256 != "":
                                    is_exploit = True
