      ...
      {activeAlert && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="border-2 border-red-600 bg-zinc-950 p-6 rounded max-w-xl w-full flex flex-col items-center text-center gap-4 shadow-[0_0_50px_rgba(239,68,68,0.25)]">
            <div className="w-16 h-16 rounded-full bg-red-950/30 border-2 border-red-500 flex items-center justify-center">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-40">
          <div className="border border-zinc-800 bg-zinc-950 p-6 rounded max-w-lg w-full flex flex-col gap-4 shadow-[0_0_30px_rgba(255,255,255,0.05)]">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              {selectedProcess.score !== undefined && (
                <div>
                  <span className="text-zinc-600 uppercase font-bold block">Threat Score</span>
                  <span className={`font-bold ${selectedProcess.score >= 70 ? "text-red-500" : "text-zinc-400"}`}>
                    {selectedProcess.score} / 100
                  </span>
                </div>
              )}
            </div>

            {selectedProcess.suspicious_dlls && selectedProcess.suspicious_dlls.length > 0 && (
              <div className="border border-red-900/50 bg-red-950/10 p-3 rounded text-xs text-red-400 font-mono">
                <span className="font-bold block uppercase mb-1">Suspicious Mapped Modules (Unsigned)</span>
                <ul className="list-disc list-inside flex flex-col gap-1 max-h-32 overflow-y-auto">
                  {selectedProcess.suspicious_dlls.map((dll, idx) => (
                    <li key={idx} className="break-all">{dll}</li>
                  ))}
                </ul>
              </div>
            )}
