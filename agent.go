...
func quarantineFile(filePath string, pid int, sha256 string) {
	if filePath == "" {
		return
	}
	os.MkdirAll("C:\\ProgramData\\ProcessSentry\\Quarantine", 0755)
	filePathClean := strings.ReplaceAll(filePath, "/", "\\")
	parts := strings.Split(filePathClean, "\\")
	fileName := parts[len(parts)-1]
	fileName = strings.ReplaceAll(fileName, "..", "")

	destPath := "C:\\ProgramData\\ProcessSentry\\Quarantine\\" + fileName + ".vir"
	metaPath := "C:\\ProgramData\\ProcessSentry\\Quarantine\\" + fileName + ".json"

	err := os.Rename(filePathClean, destPath)
	if err != nil {
		err = copyFile(filePathClean, destPath)
		if err == nil {
			os.Remove(filePathClean)
		}
	}

	metaData := map[string]interface{}{
		"original_path":   filePathClean,
		"quarantine_time": time.Now().Format("2006-01-02 15:04:05"),
		"sha256":          sha256,
		"trigger_pid":     pid,
	}
	...
	if err == nil {
		os.WriteFile(metaPath, jsonData, 0644)
	}
}
...
func socketReceiver(conn net.Conn) {
	...
	for {
		...
		if line == "ISOLATE" {
			...
			pathData := strings.TrimPrefix(line, "QUARANTINE:")
			parts := strings.Split(pathData, ";")
			path := ""
			pid := 0
			hash := ""
			if len(parts) > 0 {
