...
func sendProcessPayload(conn net.Conn, secretToken string, childPid uint32, parentPid uint32, childPath string, childName string, commandLine string) {
	connMutex.Lock()
	defer connMutex.Unlock()

	if seenPIDs[childPid] {
		return
	}
	...
	parentName := getProcessNameByPID(parentPid)
	sha256Val := calculateSHA256(childPath)
	isSignedVal := isSigned(childPath)

	println("[Agent] Intercepted process:", childName, "(PID:", childPid, ")")
...
func pollProcessLoop(conn net.Conn, secretToken string) {
	hSnapshot, _, _ := procCreateToolhelp32Snapshot.Call(TH32CS_SNAPPROCESS, 0)
	if syscall.Handle(hSnapshot) != syscall.InvalidHandle {
		var pe32 PROCESSENTRY32W
		pe32.DwSize = uint32(unsafe.Sizeof(pe32))
		if ret, _, _ := procProcess32FirstW.Call(hSnapshot, uintptr(unsafe.Pointer(&pe32))); ret != 0 {
			for {
				seenPIDs[pe32.Th32ProcessID] = true
				if ret, _, _ := procProcess32NextW.Call(hSnapshot, uintptr(unsafe.Pointer(&pe32))); ret == 0 {
					break
				}
			}
		}
	...
	for range ticker.C {
		...
		if ret, _, _ := procProcess32FirstW.Call(hSnapshot, uintptr(unsafe.Pointer(&pe32))); ret != 0 {
			for {
				...
				if !seenPIDs[childPid] {
					childName := syscall.UTF16ToString(pe32.SzExeFile[:])
					parentPid := pe32.Th32ParentProcessID
					sendProcessPayload(conn, secretToken, childPid, parentPid, "", childName, childName)
				}
...
func pollNetworkConnections(conn net.Conn, secretToken string) {
	...
	for range ticker.C {
		...
		for _, line := range lines {
			...
			destAddr := parts[2]
			destPortIdx := strings.LastIndex(destAddr, ":")
			if destPortIdx == -1 {
				continue
			}
			destIP := destAddr[:destPortIdx]
			destPort, _ := strconv.Atoi(destAddr[destPortIdx+1:])

			key := pidStr + ":" + destIP + ":" + strconv.Itoa(destPort)
			connMutex.Lock()
			isSeen := seenConnections[key]
			connMutex.Unlock()
			...
			localAddr := parts[1]
			localPortIdx := strings.LastIndex(localAddr, ":")
			if localPortIdx == -1 {
				continue
			}
			localIP := localAddr[:localPortIdx]
			localPort, _ := strconv.Atoi(localAddr[localPortIdx+1:])
...
func main() {
	...
	go pollProcessLoop(conn, secretToken)

	go pollNetworkConnections(conn, secretToken)

	println("[Agent] Starting background ETW & Kernel process monitor session...")
			...
			if err := session.AddProvider(guid); err == nil {
				cb := func(e *etw.Event) {
					if e.Header.ID == 1 {
						...
						sendProcessPayload(conn, secretToken, childPid, parentPid, childPath, "", commandLine)
					}
				}
