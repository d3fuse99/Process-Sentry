...
func main() {
	...
	for {
		...
		println("[Agent] Server unavailable ({e}). Retrying in 3 seconds...")
	...
	cb := func(e *etw.Event) {
		if e.Header.ID == 1 {
			...
			println("[Agent] Intercepted process via ETW:", childName, "(PID:", childPid, ")")
	...
	go func() {
		if err := session.Process(cb); err != nil {
			println("[Agent] Error processing ETW events:", err.Error())
		}
	}()
